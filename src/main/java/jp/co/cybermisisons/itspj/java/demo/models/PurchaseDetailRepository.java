...
public interface PurchaseDetailRepository extends JpaRepository<PurchaseDetail, Integer> {
    ...
    // @Transactional
    // @Modifying(clearAutomatically = true)
    // @Query("update PurchaseDetail set quantity =:quantity, total_amount
    // =:total_amount where id =:detail_id")
    // void updatePurchaseDetailQuantity(@Param("product_name") String product_name,
    // @Param("total_amount") Integer total_amount, @Param("detail_id") Integer
    // detail_id);
}
